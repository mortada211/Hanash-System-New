...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  // دالة تحويل تنسيق المبالغ المالية مع الفواصل
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
                ...
                return Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('${_formatCurrency(totalP)} د.ع', style: const TextStyle(fontSize: 13)),
                    ],
                  ),
                );
              }),
            ...
            _detailRow('إجمالي الفاتورة:', '${_formatCurrency(invoice.totalAmount)} د.ع'),
            _detailRow('صافي الربح:', '${_formatCurrency(invoice.profit)} د.ع', color: Colors.greenAccent),
          ],
        ),
        actions: [
  ...
  void _showDetailedReport(List<InvoiceModel> filteredInvoices) {
    ...
    // حصر مبيعات كل منتج (الكمية المباعة وإجمالي السعر)
    Map<String, int> productSoldQuantities = {};
    Map<String, double> productSoldRevenues = {};
    ...
    for (var inv in filteredInvoices) {
      ...
      // تجميع كميات وإيراد المنتجات المباعة
      for (var order in inv.orders) {
        String itemName = order['item_name'] ?? 'منتج غير معروف';
        int qty = (order['quantity'] ?? 1).toInt();
        double price = (order['unit_price'] ?? 0).toDouble();
        double totalPrice = (order['total_price'] ?? (qty * price)).toDouble();

        productSoldQuantities[itemName] = (productSoldQuantities[itemName] ?? 0) + qty;
        productSoldRevenues[itemName] = (productSoldRevenues[itemName] ?? 0) + totalPrice;
      }
    }
          ...
          builder: (context, scrollController) {
            return ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _reportCard(title: 'إجمالي المبيعات', value: '${_formatCurrency(totalSales)} د.ع', icon: Icons.monetization_on, color: Colors.blueAccent)),
                    const SizedBox(width: 10),
                    Expanded(child: _reportCard(title: 'صافي الربح', value: '${_formatCurrency(totalProfit)} د.ع', icon: Icons.account_balance_wallet, color: Colors.greenAccent)),
                  ],
                ),
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(child: _reportCard(title: 'معدل قيمة الفاتورة', value: '${_formatCurrency(avgInvoice)} د.ع', icon: Icons.show_chart, color: Colors.purpleAccent)),
                    const SizedBox(width: 10),
                ...
                Row(
                  children: [
                    Expanded(child: _reportCard(title: 'أعلى فاتورة', value: '${_formatCurrency(highestInvoice)} د.ع', icon: Icons.arrow_upward, color: Colors.tealAccent)),
                    const SizedBox(width: 10),
                    Expanded(child: _reportCard(title: 'أدنى فاتورة', value: '${_formatCurrency(lowestInvoice)} د.ع', icon: Icons.arrow_downward, color: Colors.orangeAccent)),
                  ],
                ),
                ...
                // --- قسم التقرير المخصص للمنتجات المباعة ---
                const Divider(height: 30),
                const Text('تقرير المنتجات المباعة:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.amber)),
                const SizedBox(height: 10),
                productSoldQuantities.isEmpty
                    ? const Text('لا توجد منتجات مباعة في هذه الفترة', style: TextStyle(color: Colors.white54, fontSize: 13))
                    : Column(
                        children: productSoldQuantities.entries.map((entry) {
                          final pName = entry.key;
                          final qty = entry.value;
                          final revenue = productSoldRevenues[pName] ?? 0;
                          ...
                          return Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                Row(
                                  children: [
                                    const Icon(Icons.shopping_bag, color: Colors.amber, size: 18),
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text('الكمية المباعة: $qty', style: const TextStyle(fontSize: 12, color: Colors.amberAccent)),
                                    Text('${_formatCurrency(revenue)} د.ع', style: const TextStyle(fontSize: 12, color: Colors.greenAccent, fontWeight: FontWeight.bold)),
                                  ],
                                ),
                              ],
                            ),
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text('${_formatCurrency(sales)} د.ع', style: const TextStyle(fontWeight: FontWeight.bold)),
                          ],
                        ),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text('الربح: ${_formatCurrency(profit)} د.ع', style: const TextStyle(fontSize: 11, color: Colors.greenAccent)),
                          ],
                        ),
                  ...
                  child: Row(
                    children: [
                      ...
                      ActionChip(
                        ...
                        backgroundColor: _selectedFilterLabel == 'مخصص' ? Colors.amber.withOpacity(0.3) : Colors.grey[800],
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text('${_formatCurrency(totalSales)} د.ع', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.blueAccent)),
                              ],
                            ),
                          ),
                        ),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text('${_formatCurrency(totalProfit)} د.ع', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.greenAccent)),
                              ],
                            ),
                          ),
                        ),
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                trailing: Column(
                                  ...
                                  crossAxisAlignment: CrossAxisAlignment.end,
                                  children: [
                                    Text('${_formatCurrency(inv.totalAmount)} د.ع', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blueAccent)),
                                    Text('ربح: ${_formatCurrency(inv.profit)}', style: const TextStyle(fontSize: 11, color: Colors.greenAccent)),
                                  ],
                                ),
                              ),
                            );
